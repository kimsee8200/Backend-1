...
export class NoticeController {
  ...
  async create(@Body() body: CreateNoticeDto, @UploadedFiles() files: any[]) {
    ...
    return {
      ...
      title: n.title,
      content: n.content,
      images: n.images.map((i) => i.url),
      create_at: n.createdAt.toISOString().slice(0, 10),
      update_at: n.updatedAt.toISOString().slice(0, 10),
    };
  }
  ...
  async update(
    ...
    const images = (files || []).map((f) => `/uploads/${f.filename}`);
    const n = await this.service.updateNotice(
      id,
      body.title,
      body.content,
      images,
