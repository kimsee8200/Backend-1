...
@ApiExtraModels(SuccessResponseDto, SendCodeResponseDto, ValidateCodeResponseDto)
...
export class LicenceCodeController {
    ...
    @Get('generate/:email')
    async sendLicenceCode(@Param('email') email: string): Promise<SendCodeResponseDto> {
    ...
    @Post('validate')
    async validateLicenceCode(@Body() body: SendCodeDto): Promise<ValidateCodeResponseDto> {
