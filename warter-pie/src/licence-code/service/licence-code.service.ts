...
export class LicenceCodeService {
    ...
    private async generateLicenceCode(email: string): Promise<string> {
        ...
        const licenceCode = Math.random().toString(36).substring(2, 15).toUpperCase();
        this.codes.add([email, licenceCode]);
        return licenceCode;
    }

    async validateLicenceCode(email: string, licenceCode: string): Promise<boolean> {
        // Logic to validate the licence code
        const codeEntry = Array.from(this.codes).find(([e, c]) => e === email && c === licenceCode);
        if (codeEntry) {
    ...
    async sendLicenceCode(email: string): Promise<void> {
        try {
            ...
            if (!process.env.MAIL_PASS) {
                this.logger.log(`[개발 모드] 인증 코드가 생성되었습니다: ${licenceCode}`);
            ...
            throw new Error('이메일 전송에 실패했습니다. 앱 비밀번호를 확인해주세요.');
        }
    }
}
